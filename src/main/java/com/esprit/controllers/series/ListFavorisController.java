...
public class ListFavorisController implements Initializable {
     ...
     *
     * @param url            location used to resolve relative paths for the root
     *                       object, may be null
    ...
    /**
     * Render the provided favorite Series objects into the ListView and attach
     * per-item controls for like, dislike, favorite, and watch.
     *
     * @param series the list of Series to display; each element becomes a ListView
     *               cell with interactive controls that update and persist user
     *               actions
     */
    public void afficherliste(final List<Series> series) {
        ...
        this.listViewFav.setCellFactory(param -> new ListCell<Series>() {
            /**
             * Populate the cell's graphic to display the given Series and attach UI
             * controls
             * for liking, disliking, favoriting, and watching that series.
             *
             * The method updates the cell to show the series' image, title and current
             * like/dislike
             ...
             *
             * @param item  the Series to display in the cell; may be null when the cell is
             *              empty
            ...
            protected void updateItem(final Series item, final boolean empty) {
                ...
                if (empty || null == item) {
                    ...
                    likeButton.setOnAction(new EventHandler<ActionEvent>() {
                        /**
                         * Toggle the liked state of the associated series item, update its like count
                         * and UI state, and persist the change.
                         *
                         * @param event the ActionEvent that triggered this handler
                         * @throws RuntimeException if a database error occurs while updating the like
                         *                          state
                         */
                        @Override
                        /**
                    ...
                    dislikeButton.setOnAction(new EventHandler<ActionEvent>() {
                         ...
                         *
                         * Updates the item's click and dislike counters, persists the dislike state to
                         * the database,
                         * and enables or disables the like button to reflect the new state.
                         *
                         * @throws RuntimeException if a database error occurs while persisting the
                         *                          dislike state
                         */
                        @Override
                        /**
                    ...
                    favButton.setOnAction(new EventHandler<ActionEvent>() {
                        /**
                         * Toggle the favorite state for the current series and persist the change for
                         * user ID 1.
                         *
                         * Increments the series' internal favorite-click counter and either creates or
                         * deletes a Favorite
                         * record for the session user and series; database errors are propagated as a
                         * RuntimeException.
                         *
                         * @param event the ActionEvent from the favorite button
                         * @throws RuntimeException if a database error occurs while creating or
                         ...
                         */
                        public void handle(final ActionEvent event) {
                            final Client id_user = (Client) SessionManager.getCurrentUser();
                            final Series id_serie = new SeriesService().getSeriesById(item.getId());
                            ...
                            try {
                                if ((0 == item.getClickFavorites()) || (0 != item.getClickFavorites() % 2)) {
                                    ...
                                    final Favorite fav = sf.getByIdUserAndIdSerie(id_user.getId(), id_serie.getId());
                                    sf.delete(fav);
     ...
     *
     * @throws SQLException if retrieving favorites or series from the data layer
     *                      fails
     */
    @FXML
    private void loadSeriesFavList() throws SQLException {
        ...
        for (int i = 0; i < listFav.size(); i++) {
            final Series s = ss.getByIdSeries(listFav.get(i).getSeries().getId());
            listSerieById.add(s);
        }
