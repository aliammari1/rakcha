...
public class DetailsProductClientController implements Initializable {
    ...
    /**
     * Set the current product ID and initialize the product details view for that
     * product.
     *
    ...
    /**
     * Initialize the controller by loading the top-three accepted products and
     * wiring the back icon to show the product list.
     *
    ...
    /**
     * Load the product identified by this.produitId and populate the UI with its
     * detail and shopping-cart cards.
     * <p>
     * Clears existing children from the detail FlowPane, adds a product detail card
     * and a shopping-cart card when the
     * product is found, and logs an informational message if no product matches the
     * ID.
     */
    ...
    /**
     * Create an HBox containing a product detail card with image, name,
     * description,
     ...
     * of the product to the shopping cart and updates the visible cart/detail
     * panes.
     *
     * @param produit the Product to display (used to populate image, text, price,
     *                and rating)
    ...
    /**
     * Add the specified product and quantity to the shopping cart when stock is
     * available.
     * <p>
     * Creates and persists a ShoppingCart entry associated with the product and a
     * user (ID 4),
     * then updates the shopping cart view. If available stock is insufficient, logs
     * an informational message and does not modify the cart.
     *
    ...
    private void ajouterAuShoppingCart(final long produitId, final int quantity) {
        ...
        if (produitService.checkAvailableStock(produitId, quantity)) {
            ...
            shoppingcart.setUser(SessionManager.getCurrentUser());
    ...
    /**
     * Open the product listing view as a modal window, wait for it to close, then
     * close the current window.
     * <p>
     * Loads the product listing FXML, shows it in an application-modal stage owned
     ...
     * <p>
     * Makes the shopping cart and details FlowPanes visible and sets the details
     * pane opacity to 0.2.
     */
    ...
    /**
     * Creates an HBox containing a shopping-cart card for the given product with
     * image, name, price, quantity, total, and action controls.
     *
     ...
     *
     * @param shoppingcartContainer the HBox that contains the shopping cart panel
     *                              being closed
     */
     ...
     * "ShoppingCart des produits", and closes the stage that contains the event's
     * source.
     *
     * @param event the MouseEvent that triggered this navigation; its source is
     *              used to locate and close the current window
     */
    @FXML
    ...
    /**
     * Opens the product comments view in a new window and closes the current
     * window.
     *
    ...
    /**
     * Opens the product comment view (CommentaireProduct.fxml) in a new window
     * titled "cinema" and closes the current window.
     */
    @FXML
    ...
    /**
     * Open the "AffichageEvenementClient" view in a new window and close the
     * current window.
     *
     * @param event the ActionEvent whose source node is used to obtain and close
     *              the current Stage
     */
    @FXML
     ...
     * <p>
     * Loads "AfficherProductClient.fxml", attaches it to a new Stage titled
     * "products",
     * shows that stage, and closes the stage obtained from the provided event's
     * source.
     *
    ...
    /**
     * Opens the film user view (filmuser.fxml) in a new window and closes the
     * current window.
     *
    ...
    /**
     * Load and display the top three best-selling products with accepted status
     * into the controller's topthreeVbox.
     *
     * <p>
     * The method queries ProductService for products ordered by quantity and
     * status, creates a UI card for each
     * of the first three products via createtopthree(...), and appends those cards
     * to topthreeVbox. If fewer than
     * three products are available, the method logs an informational message and
     * returns without modifying the UI.
     * Any exceptions encountered during loading are logged at SEVERE level.
     * </p>
     */
    ...
    /**
     * Creates a compact top-three product card showing the product image, name, and
     * price.
     * <p>
     * The card displays a product image (falls back to a default image if the
     * product image is unavailable)
     * and shows the product name and price. Clicking the image or the name opens
